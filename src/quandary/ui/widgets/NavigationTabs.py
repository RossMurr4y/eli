...
class NavigationTabs(TabbedContent):
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        debug_panel.append(
            str(QuandaryLog(QuandaryLogType.DEBUG, event.input.value))
        )
        ...
        debug_panel.append(
            str(QuandaryLog(QuandaryLogType.DEBUG, str(self.mode.response)))
        )
        # debug_panel.append(str(self.mode.response))
        # display answer
        response_panel.append_on_new_line(
            "> 🗣️ " + str(self.mode.response.value)
        )
    ...
    def on_switch_setting_changed(self, event: SwitchSetting.Changed) -> None:
        ...
        debug_panel.append(
            str(
                QuandaryLog(
                    QuandaryLogType.DEBUG,
                    [{"id": event.id, "enabled": event.enabled}],
                )
            )
        )
